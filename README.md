...
AI：DashScope（阿里云百炼）
  ...
  ├── qwen-image-edit     — 单品自动抠图去背景
  ├── qwen-vl-max         — 多模态穿搭评分 + AI 自动识别分类
  └── qwen-max            — 智能搭配推荐
天气：和风天气 API（3 天预报 + 穿衣指数，按小时缓存到 SQLite）
```
...
- [x] AI 五维评分 + 雷达图
- [x] 重新打分功能
- [x] 天气联动（3 天预报 + 穿衣建议 + 目标日期智能选择）
- [x] 城市搜索（900+ 城市本地数据）
- [x] 天气数据库缓存（按城市+小时，30 天自动过期）
- [x] Prompt 集中管理 + tryon 代码去重
- [ ] 推荐 prompt 精调与效果优化
